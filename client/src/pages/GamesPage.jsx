...
export default function GamesPage() {
  ...
  useEffect(() => {
    const fetchCatalog = async () => {
      try {
        // 1. Fetch live featured/trending games from the backend
        ...
        // 2. Extract numeric Steam IDs from COMPREHENSIVE_CATALOG
        const steamAppIds = COMPREHENSIVE_CATALOG
          .filter(g => g.platform === 'Steam' && /^\d+$/.test(g._id))
          .map(g => g._id);

        let catalogWithLivePrices = [...COMPREHENSIVE_CATALOG];

        // 3. Request live pricing for catalog games if your endpoint supports batch/lookup
        try {
          // Pass country code &cc=us to ensure uniform USD prices
          const priceRes = await publicApi.get(`/steam/prices?appids=${steamAppIds.slice(0, 30).join(',')}&cc=us`);
          const priceMap = priceRes.data?.data || priceRes.data || {};

          catalogWithLivePrices = COMPREHENSIVE_CATALOG.map(game => {
            if (priceMap[game._id]) {
              const info = priceMap[game._id];
              return {
                ...game,
                price: info.final_formatted || (info.final ? `$${(info.final / 100).toFixed(2)}` : game.price)
              };
            }
            return game;
          });
        } catch {
          // Fall back to predefined catalog values if the prices endpoint is not ready
        }

        // 4. Combine live Steam list and updated catalog
        setGames([...liveList, ...catalogWithLivePrices]);
      } catch (err) {
  ...
  const NSFW_TERMS = ['sex', 'hentai', 'milf', 'porn', 'erotic', 'nudity', 'adult', 'nsfw', 'succubus', 'lewd', 'boobs', 'demon hunters'];

  const filteredGames = useMemo(() => {
    return games.filter(game => {
      const title = (game.title || game.name || '').toLowerCase();
      const desc = (game.short_description || game.shortDescription || '').toLowerCase();

      // Check if any forbidden keyword appears anywhere inside title or description
      const isNSFW = NSFW_TERMS.some(term => title.includes(term) || desc.includes(term));
